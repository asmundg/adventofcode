...
def parse(fname: str) -> List[Node]:
    with open(fname, encoding="utf-8") as handle:
        nums = [Node(num=int(line)) for line in handle.read().strip().split("\n")]
...
def reindex(nums: List[Node]) -> List[Node]:
    """
...
def mix(nums: List[Node]) -> List[Node]:
